...
```bash
find /path/to/nixos/modules/egrabber/egrabber-drivers -type f \
  \( -name "*.c" -o -name "*.h" -o -name "Makefile*" -o -name "*.mk" \) \
  -exec sed -i 's/\r$//' {} +

find /path/to/nixos/modules/egrabber/memento-drivers -type f \
  \( -name "*.c" -o -name "*.h" -o -name "Makefile*" -o -name "*.mk" \) \
  -exec sed -i 's/\r$//' {} +
```
...
```bash
cd ~/Downloads/egrabber-linux-x86_64-26.02.1.18
euresys-fhs -c "sudo bash install.sh"

cd ~/Downloads/memento-linux-x86_64-26.02.0.8
euresys-fhs -c "sudo bash install.sh"
```
