                ...
                for (int interfaceIndex = 0; interfaceIndex < 4; ++interfaceIndex) {
                    for (int deviceIndex = 0; deviceIndex < 8; ++deviceIndex) {
                        try {
                            ...
                            grabbers.push_back(new Grabber(genTL, interfaceIndex, deviceIndex));
...
static void sample() {
    ...
    grabbers.runScript(Tools::getSampleFilePath("210-show-all-grabbers.show.js"));
}

static Tools::DeprecatedSample addSample(__FILE__, sample, "Show available grabbers\n"
