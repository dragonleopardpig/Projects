...
static void sample() {
    ...
    grabber.setInteger<StreamModule>("DmaSegmentOffset", -1);
    grabber.execute<StreamModule>("DmaAddSegment");
    // The ROI data of the even lines is stored from the beginning of the buffer
    ...
    grabber.setInteger<StreamModule>("DmaSegmentOffset", -1);
    grabber.execute<StreamModule>("DmaAddSegment");
    // The ROI data of the odd lines is stored from the middle of the buffer
    grabber.setInteger<StreamModule>("DmaSegmentSize", roiWidth * sizeOfPixel);
    grabber.setInteger<StreamModule>("DmaSegmentOffset", roiWidth * sizeOfPixel * roiHeight / 2);
    grabber.setInteger<StreamModule>("DmaSegmentOffsetIncrement", grabber.getInteger<StreamModule>("DmaSegmentSize"));
    grabber.execute<StreamModule>("DmaAddSegment");
    // The remaining data on the right of the ROI is discarded
    grabber.setInteger<StreamModule>("DmaSegmentSize", (width - roiWidth - roiX) * sizeOfPixel);
    grabber.setInteger<StreamModule>("DmaSegmentOffset", -1);
    grabber.execute<StreamModule>("DmaAddSegment");
    // The sequence of segments above is repeated "half the height of the ROI" times
    grabber.setInteger<StreamModule>("DmaSegmentsRepeatCount", roiHeight / 2);
    grabber.execute<StreamModule>("DmaAddSequence");
...
static Tools::Sample addSample(__FILE__, sample, "Grab N frames but store a deinterlaced smaller region in the user buffers");
